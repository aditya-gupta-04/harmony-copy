...
def calculate_score(dataset_name, model, z_dim, pixel, batch_size, device):
    train_loader, test_loader, _, _ = data_loader(dataset_name, pixel, batch_size, shuffle=False)
    save_latent_variables(dataset_name, train_loader, model, 'train', pixel, z_dim, batch_size, device=device)
